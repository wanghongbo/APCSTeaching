 ...
 * using the enhanced Java for-each loop. See getMolecularWeight in MoleculeAnalyzer for an example.
 */
public final class BondedAtom implements Iterable<BondedAtom> {
    ...
    public Iterator<BondedAtom> iterator() {
        loadConnectedAtoms();
        return connectedAtoms.stream().map(BondInfo::getAtom).iterator();
    }
     ...
     *
     * @return an iterator of BondInfo objects describing our neighbor atoms.
     */
