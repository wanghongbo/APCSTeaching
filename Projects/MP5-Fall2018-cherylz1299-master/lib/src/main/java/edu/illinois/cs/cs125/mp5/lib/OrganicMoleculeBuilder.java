...
public abstract class OrganicMoleculeBuilder {
    ...
    /**
     * How many bonds connect each carbon to the next one in the backbone.
     */
    ...
    public OrganicMoleculeBuilder(final int setCoreCarbons) {
        ...
        bondCounts = new int[setCoreCarbons - 1];
        Arrays.fill(bondCounts, 1);
    }
            ...
            switch (type) {
                case ALKANE:
                    ...
                    carbonAttachments[alkylLength - 1][1] =
                        createHalogen(ChemicalElement.HYDROGEN)
                            .attach(carbons[alkylLength - 1]);
